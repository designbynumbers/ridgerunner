    ...
    for(step = 0; step < nsteps; step++) {
		 ...
		 pass B to dgelss. */
		  ...
		  //dgelss_(&M, &N, &NRHS, A, &LDA, B, &LDB, S, &RCOND, &RANK, WORK, &LWORK, &INFO);
			...
			double* rowOrdered = (double*)malloc(sizeof(double)*M*N);
			{
				int rItr, cItr;
				for( rItr=0; rItr<M; rItr++ )
				{
					...
					{
						rowOrdered[rItr*N + cItr] = clapack_matrix_read(A, LDA, rItr, cItr);
					}
				}
			}
			taucs_ccs_matrix* Af = taucs_construct_sorted_ccs_matrix(rowOrdered, N, M);
			free(rowOrdered);
			
			clapack_matrix_print(A, M, N);
			taucs_print_ccs_matrix(Af);
			
			{
				...
				lsqr_in->cond_lim = 0;
				lsqr_in->max_iter = lsqr_in->num_rows + lsqr_in->num_cols + 50;
				...
				for( bItr=0; bItr<Af->n; bItr++ ) // not really bItr here, but hey
					B[bItr] = lsqr_out->sol_vec->elements[bItr];
