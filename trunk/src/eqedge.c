     ...
     
{
  ...
  int i;
  double *A;
  plc_vector LeftTan,RightTan,EdgeDir;
  ...
  *LDA = plc_strand_edges(&L->cp[comp]);
  *N   = L->cp[comp].nv;
  ...
  for(i=0;i<L->cp[comp].nv-1;i++) {
    ...
    LeftTan = plCurve_tangent_vector(L,comp,i);
    RightTan = plCurve_tangent_vector(L,comp,i+1);
    EdgeDir = plCurve_edge_dir(L,comp,i);

    clapack_matrix_write( -plc_M_dot(EdgeDir,LeftTan) ,A,*LDA,i,i  );
    clapack_matrix_write( plc_M_dot(EdgeDir,RightTan),A,*LDA,i,i+1);
  ...
  if (!L->cp[comp].acyclic) {
    ...
    LeftTan = plCurve_tangent_vector(L,comp,i);
    RightTan = plCurve_tangent_vector(L,comp,0);
    EdgeDir = plCurve_edge_dir(L,comp,i);

    clapack_matrix_write( -plc_M_dot(EdgeDir,LeftTan) ,A,*LDA,i,i);
    clapack_matrix_write( plc_M_dot(EdgeDir,RightTan),A,*LDA,i,0);
...
double *edgelength_error(plCurve *L,int comp, search_state* inState)
	...
	the "target" value of plc_strand_length/plc_strand_edges,
...

{
  ...
  edges = plc_strand_edges(&(L->cp[comp]));
  ...
  for(i=0;i<edges;i++) {
    ...
    V = L->cp[comp].vt[i];
    plc_M_sub_vect(V,L->cp[comp].vt[i+1]);      
    Err[i] = plc_M_norm(V);
  ...
  for(i=0;i<edges;i++) {
   ...
   // Err[i] -= target_length;
    Err[i] -= inState->conserveLength[i+inState->compOffsets[comp]];
	...
	
{
  ...
  for(comp = 0;comp < L->nc; comp++) {
    ...
    target_length = plc_strand_length(&L->cp[comp])/
      (double)(plc_strand_edges(&L->cp[comp]));
	...
	fprintf(stderr,"    maxErr before step %d = %g.\n",
		step,maxError(L,comp,inState));
	fprintf(stderr,"    max/min before step %d = %g.\n\n",
		step,plCurve_long_edge(L)/plCurve_short_edge(L));
      ...
      A = (__CLPK_doublereal *)
	(tangential_rigidity_matrix(L,comp,(int *)(&LDA),(int *)(&N)));
      
      /* 2) Find the edge length errors. */

      Err = (__CLPK_doublereal *)(edgelength_error(L,comp,inState));
      ...
      /* We now set up the RHS B. Paradoxically, though B is a vector
	 of length M (# edges), we must allocate a vector of length
	 _N_ (3 * # verts) because this space will be overwritten by
	 the least-squares _solution_ to the problem.
	 
	 We similarly set LDB = N even though there are only M nonzero
	 elements when we pass B to dgelss. */
	...
	B[i] = Err[i];
      ...
      double* newA = (double*)calloc(M*N,sizeof(double));
      int rItr, cItr;
      
      for( rItr=0; rItr<M; rItr++ )
	{
	  for( cItr=0; cItr<N; cItr++ )
	    newA[rItr*N + cItr] = clapack_matrix_read(A, LDA, rItr, cItr);
	}
      
      reloadDump(A, M, N, B, Err);
	...
	if (INFO < 0) {
	  ...
	  fprintf(stderr,"lapack_eqedge: cDGELSS reports " \
		  "illegal %d th argument.\n",(int)(-INFO));
	  exit(1);

	} else {
	  
	  fprintf(stderr,"lapack_eqedge: cDGELSS couldn't " \
		  "find SVD for rigidity matrix.\n");
	  exit(1);
	...
	stepV = plCurve_tangent_vector(L,comp,i);
	plc_M_scale_vect(B[i],stepV);
	plc_M_sub_vect((L->cp[comp].vt[i]),stepV);
	...
	fprintf(stderr,"   Maximum error after step    %g.\n",
		maxError(L,comp,inState));
	fprintf(stderr,"   max/min after step          %g.\n\n",
		plCurve_long_edge(L)/plCurve_short_edge(L));
     ...
     
{
  ...
  Err = edgelength_error(L,comp,inState);
  
  for(i=0;i<plc_strand_edges(&L->cp[comp]);i++) {
