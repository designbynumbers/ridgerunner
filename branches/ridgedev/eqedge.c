...
void    our_matrix_write(double val, double *A, int cols, int i, int j)
...
//  A[j*LDA + i] = val;
		...
		// change for taucs conversion ordering
		A[i*cols + j] = val;
}

double  clapack_matrix_read(double *A, int cols, int i, int j)
...

{
  ...
  for(i=0;i<M;i++) {
    ...
    for(j=0;j<N;j++) {
      ...
      printf("%3.2g ",clapack_matrix_read(A,N,i,j));
...

{
  ...
  *N   = L->cp[comp].nv-1; /// we do this to fix one vertex to ensure the least squares 
						 // problem can be solved with taucs_lsqr which requires
						 // everything to be positive definite

  A = calloc( (*LDA) * ((*N)+1), sizeof(double) );
  ...
  for(i=0;i<L->cp[comp].nv-1;i++) {
    ...
    our_matrix_write( -octrope_dot(EdgeDir,LeftTan) ,A,*N,i,i  );
    our_matrix_write( octrope_dot(EdgeDir,RightTan),A,*N,i,i+1);
  ...
  if (!L->cp[comp].acyclic) {
		...
		our_matrix_write( -octrope_dot(EdgeDir,LeftTan) ,A,*N,i,i);
		our_matrix_write( octrope_dot(EdgeDir,RightTan),A,*N,i,0);
...
//		clapack_matrix_print(A, M, N);
		  		  
//		  dgelss_(&M, &N, &NRHS, A, &LDA, B, &LDB, S, &RCOND, &RANK, WORK, &LWORK, &INFO);

			INFO = 0;
			sparseA = taucs_construct_sorted_ccs_matrix(A, N, M);
//			taucs_print_ccs_matrix(sparseA);
			x = t_lsqr(sparseA, A, B);
			
		//	reloadDump(A, M, N, x, B);
